...
export function registerAffiliateRoutes(app: Express) {
  app.get("/api/affiliate/dashboard", async (req: Request, res: Response) => {
    ...
    res.json({
      uniqueHash,
      referralLink: `https://trusthome.tlid.io/ref/${uniqueHash}`,
      tier: computeTier(0),
  ...
  app.get("/api/affiliate/link", async (req: Request, res: Response) => {
    ...
    res.json({
      uniqueHash,
      links: {
        trusthome: `https://trusthome.tlid.io/ref/${uniqueHash}`,
  ...
  app.post("/api/affiliate/track", async (req: Request, res: Response) => {
    ...
    res.json({ success: true, message: "Referral tracked", app: "trusthome" });
  });
  ...
  console.log("[AFFILIATE] TrustHome affiliate routes registered");
}
